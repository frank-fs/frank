...
| Marker | File | AT | Replaces |
|--------|------|----|----------|
| FRANK-STUB(AT-S5) | sample/TicTacToe-v732/GeneratedLinkedData.fs | AT-S5 | Replace with `frank-semantic` MSBuild codegen (LD-U2) |

The `Frank.Discovery` middleware (OPTIONS/Allow/Link describedby, ALPS, JSON Home)
is REAL. The artifact it consumes is now GENERATED by MSBuild (`FrankGenerateSemantic`
target) from `.frank/semantic-mappings.lock.json` via `Frank.Cli.MSBuild`.
