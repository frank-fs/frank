...
echo ""
echo "=== Testing Existing Examples (FR-009) ==="

echo -n "1. displayDate: "
RESULT=$(curl -s -m 2 "$BASE_URL/displayDate" | head -1)
if echo "$RESULT" | grep -q "datastar-patch-elements"; then
    echo "PASS"
else
    echo "FAIL"
fi

echo -n "2. searchItems (q=app): "
RESULT=$(curl -s -m 2 "$BASE_URL/search?q=app")
...
echo -n "3. greetUser: "
RESULT=$(curl -s -m 2 "$BASE_URL/greet?name=Test")
...
echo -n "4. loadProducts (electronics): "
RESULT=$(curl -s -m 2 "$BASE_URL/products?category=electronics")
...
echo -n "5. clock: "
RESULT=$(curl -s -m 2 "$BASE_URL/clock")
if echo "$RESULT" | grep -q "clock"; then
    echo "PASS"
else
    echo "FAIL"
fi

echo -n "6. loadItemsPage: "
RESULT=$(curl -s -m 2 "$BASE_URL/load-items/1")
if echo "$RESULT" | grep -q "items-container"; then
    echo "PASS"
else
    echo "FAIL"
fi

echo -n "7. dashboardRefresh: "
RESULT=$(curl -s -m 2 "$BASE_URL/dashboard/refresh")
...
echo -n "8. viewProfile (can edit): "
RESULT=$(curl -s -m 2 "$BASE_URL/profile/123")
if echo "$RESULT" | grep -q "Edit Profile"; then
    echo "PASS"
else
    echo "FAIL"
fi

echo -n "9. viewProfile (view only): "
RESULT=$(curl -s -m 2 "$BASE_URL/profile/456")
if echo "$RESULT" | grep -q "View only"; then
    echo "PASS"
else
    echo "FAIL"
fi

echo -n "10. counter (increment via POST): "
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE_URL/increment" \
