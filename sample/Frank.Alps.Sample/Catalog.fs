module Frank.Alps.Sample.Catalog
...
/// `internal` (not `private`) so `PingPong.fs`'s glue code can reuse this SAME store instance --
/// see the comment above `pingPongBaseUri` in PingPong.fs for why sharing one store across both
/// protocols is a deliberate judgment call, not an oversight.
let internal store: IProvenanceStore =
...
/// INTO (closedState's own `def` IRI) -- see stateResolver above for why that typing is
...
/// primary representation (bound to viewGame via `binds`, so `Alps.excerpt` and
...
/// representation.
///
/// `internal` (not `private`) so `Program.fs`'s `main` can wire this into the webHost.
let internal gameResource =
    resource "/games/{id}" {
        link (fun ctx ->
            Seq.singleton
