...
type RdfVerificationTests() =
    ...
    // ── AT-R3: PROV-O parses to real prov triples ────────────────────────────

    [<Test>]
    member this.``AT-R3 PROV-O parses to real prov triples via Link rel``() =
        task {
            use! ctx = this.NewContext()
            let gameId = "at-r3-" + Guid.NewGuid().ToString("N")[..7]
            let originBase = Server.Url().TrimEnd('/')
            let gameUrl = sprintf "/games/%s" gameId
            let moveUrl = sprintf "/games/%s/moves" gameId
            let provRel = "http://www.w3.org/ns/prov#has_provenance"
            let agentIri = "https://schema.org/agent"
            let squareIri = originBase + "/tictactoe#square"
            let classIri = "https://schema.org/MoveAction"

            // Initialize game
            let! _ = ctx.GetAsync gameUrl

            // Play X move → TopLeft
            let xMove = Dictionary<string, obj>()
            xMove.["@type"] <- classIri
            xMove.[agentIri] <- "X"
            xMove.[squareIri] <- "TopLeft"
            ...
            Assert.That(xResp.Status, Is.EqualTo 200, "X move failed")

            // Play O move → TopCenter
            let oMove = Dictionary<string, obj>()
            oMove.["@type"] <- classIri
            oMove.[agentIri] <- "O"
            oMove.[squareIri] <- "TopCenter"
            ...
            Assert.That(oResp.Status, Is.EqualTo 200, "O move failed")

            // Game GET also carries has_provenance link
            let! gameResp = ctx.GetAsync gameUrl
            let gameRels = RdfVerificationTests.ParseLinkRels gameResp
            Assert.That(gameRels.ContainsKey provRel, Is.True, "Game GET missing has_provenance Link")

            // Use last move response's has_provenance link to discover the moves provenance URL
            let moveRels = RdfVerificationTests.ParseLinkRels oResp
            Assert.That(moveRels.ContainsKey provRel, Is.True, "Move POST missing has_provenance Link")
            let provUrl = moveRels.[provRel]

            // GET provenance endpoint
            let! provResp = ctx.GetAsync provUrl
            Assert.That(provResp.Status, Is.EqualTo 200, "Provenance endpoint not 200")

            let contentTypeProv =
                match provResp.Headers.TryGetValue "content-type" with
                | true, v -> v
                | _ -> ""

            Assert.That(
                contentTypeProv.Contains "application/ld+json",
                Is.True,
                sprintf "Provenance response must be ld+json, got: %s" contentTypeProv
            )

            let! provBody = provResp.TextAsync()
            use provGraph = RdfVerificationTests.ParseJsonLd provBody

            // Count distinct prov:Activity subjects = number of moves played (2)
            let provActivity = "http://www.w3.org/ns/prov#Activity"
            let activities = RdfVerificationTests.SubjectsByType(provGraph, provActivity)

            Assert.That(
                activities.Length,
                Is.EqualTo 2,
                sprintf "Expected exactly 2 prov:Activity subjects (one per move), got %d" activities.Length
            )

            // Per activity: wasAssociatedWith → IRI (not blank), startedAtTime → xsd:dateTime
            let provAssoc = "http://www.w3.org/ns/prov#wasAssociatedWith"
            let provStarted = "http://www.w3.org/ns/prov#startedAtTime"
            let xsdDateTime = "http://www.w3.org/2001/XMLSchema#dateTime"

            for actSubj in activities do
                let assocNodes =
                    RdfVerificationTests.ObjectsFor(provGraph, actSubj, provAssoc) |> Seq.toList

                Assert.That(
                    assocNodes,
                    Is.Not.Empty,
                    sprintf "Activity %s missing prov:wasAssociatedWith" (actSubj.ToString())
                )

                match assocNodes.[0] with
                | :? IUriNode -> ()
                | n ->
                    Assert.Fail(sprintf "prov:wasAssociatedWith must be IRI node, got %A for %s" n (actSubj.ToString()))

                let startedNodes =
                    RdfVerificationTests.ObjectsFor(provGraph, actSubj, provStarted) |> Seq.toList

                Assert.That(
                    startedNodes,
                    Is.Not.Empty,
                    sprintf "Activity %s missing prov:startedAtTime" (actSubj.ToString())
                )

                match startedNodes.[0] with
                | :? ILiteralNode as l ->
                    Assert.That(
                        l.DataType.AbsoluteUri,
                        Is.EqualTo xsdDateTime,
                        sprintf "prov:startedAtTime datatype must be xsd:dateTime for %s" (actSubj.ToString())
                    )
                | n -> Assert.Fail(sprintf "prov:startedAtTime must be literal, got %A for %s" n (actSubj.ToString()))

            // Timestamps non-decreasing in graph order
            let times =
                activities
                |> List.choose (fun subj ->
                    RdfVerificationTests.ObjectsFor(provGraph, subj, provStarted)
                    |> Seq.tryHead
                    |> Option.bind (fun n ->
                        match n with
                        | :? ILiteralNode as l ->
                            match DateTimeOffset.TryParse l.Value with
                            | true, dt -> Some dt
                            | _ -> None
                        | _ -> None))
                |> List.sort

            Assert.That(times.Length, Is.EqualTo 2, "Expected 2 timestamp values from prov:startedAtTime")

            let timesInOrder = times |> List.pairwise |> List.forall (fun (a, b) -> a <= b)
            Assert.That(timesInOrder, Is.True, "Activity timestamps must be non-decreasing")

            // Agent values (schema:agent body attribute) must be X then O in time order
            let activitiesByTime =
                activities
                |> List.sortBy (fun subj ->
                    RdfVerificationTests.ObjectsFor(provGraph, subj, provStarted)
                    |> Seq.tryHead
                    |> Option.bind (fun n ->
                        match n with
            ...
            let agentValues =
                activitiesByTime
                |> List.choose (fun subj ->
                    RdfVerificationTests.ObjectsFor(provGraph, subj, agentIri)
                    |> Seq.tryHead
                    |> Option.map RdfVerificationTests.NodeString)

            Assert.That(agentValues.Length, Is.EqualTo 2, "Expected 2 schema:agent values (one per activity)")

            Assert.That(agentValues.[0], Is.EqualTo "X", "First activity (by time) must be X")
            Assert.That(agentValues.[1], Is.EqualTo "O", "Second activity (by time) must be O")
        }
