...
type internal SseDataLineWriter
    (bufferWriter: IBufferWriter<byte>, dataLineType: byte[], cancellationToken: CancellationToken) =
    inherit TextWriter()
    ...
    let emitLine () =
        if position > 0 then
            ...
            bufferWriter
            |> ServerSentEvent.writeUtf8Literal ServerSentEvent.dataPrefix
            |> ignore
    ...
    override _.Write(value: char) =
        ...
        | _ ->
            if position >= charBuffer.Length then
                ensureCapacity ()
