...
/// Precondition: moduleName must be non-whitespace and namespace-qualified (contain a '.').
/// Call at module boundary before any formatting.
let validateModuleName (moduleName: string) : unit =
    if System.String.IsNullOrWhiteSpace moduleName then
        invalidArg (nameof moduleName) "moduleName must not be null or whitespace"
