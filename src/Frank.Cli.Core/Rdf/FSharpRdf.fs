...
module FSharpRdf =
    ...
    let createGraph () : IGraph = new Graph() :> IGraph

    let createUriNode (graph: IGraph) (uri: Uri) : INode = graph.CreateUriNode(uri)
    ...
    let createBlankNode (graph: IGraph) (id: string) : INode = graph.CreateBlankNode(id)
    ...
    let triplesWithSubject (graph: IGraph) (node: INode) : Triple seq = graph.GetTriplesWithSubject(node)

    let triplesWithPredicate (graph: IGraph) (node: INode) : Triple seq = graph.GetTriplesWithPredicate(node)
    ...
    let toRdfNode (node: INode) : RdfNode =
        ...
        | :? ILiteralNode as l ->
            let dt = if isNull l.DataType then None else Some l.DataType
