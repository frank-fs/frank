...
let private duDecl (bases: Set<string>) (mapped: MappedResource list) =
    let iriClauses =
        mapped
        |> List.map (fun m -> m.LocalName, EmitterShared.uriExprFor bases m.ClassIri)

    let clrTypeClauses =
        mapped
        |> List.map (fun m -> m.LocalName, AstRender.rawExpr (clrTypeExpr m.FSharpType m.GenericArity))

    AstRender.unionDeclWithMembers
        "SemanticResource"
        (mapped |> List.map (fun m -> m.LocalName))
        iriClauses
        clrTypeClauses

let private caseFns (bases: Set<string>) (mapped: MappedResource list) =
    mapped
    |> List.choose (fun m ->
        if m.Cases.IsEmpty then
            ...
            let mappedClauses =
                ...
                |> List.map (fun c ->
                    ...
                    pat, AstRender.appExpr "Some" (AstRender.parenExpr (EmitterShared.uriExprFor bases c.Iri)))
...
///
/// Class/case IRIs for declared-only/owned prefixes (EmitterShared.declaredOnlyBases, #396)
/// are emitted host-relative — resolved against a live request origin at the caller's own
/// point of use via Frank.UriResolution.resolveAgainst, mirroring DiscoveryEmitter/
/// LinkedDataEmitter (#415). Genuinely external vocab IRIs (schema.org, Wikidata) stay
/// absolute, unchanged.
///
...
let emit (moduleName: string) (registry: VocabularyRegistry) (lock: LockFile) : Result<string, string> =
    ...
    |> Result.bind (fun model ->
        ...
        if mapped.IsEmpty then
            ...
            let decls = duDecl bases mapped :: caseFns bases mapped
            Ok(AstRender.formatModule moduleName (Some AstRender.autoGeneratedHeader) [] decls))
