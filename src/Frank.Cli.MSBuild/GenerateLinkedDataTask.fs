...
/// MSBuild task: evaluates the project's vocabulary CE IN-PROCESS via FSI, then
/// calls LinkedDataEmitter to write GeneratedLinkedData.fs to the intermediate output
/// directory. No child dotnet/msbuild processes are spawned.
type GenerateLinkedDataTask() =
    ...
    override this.Execute() =
        if String.IsNullOrWhiteSpace this.LockFilePath then
            this.Log.LogError("GenerateLinkedDataTask: LockFilePath must not be empty.")
            false
        elif String.IsNullOrWhiteSpace this.OutputPath then
            this.Log.LogError("GenerateLinkedDataTask: OutputPath must not be empty.")
            false
        elif String.IsNullOrWhiteSpace this.ModuleName then
            this.Log.LogError("GenerateLinkedDataTask: ModuleName must not be empty.")
            false
        elif this.SourceFiles.Length = 0 then
            this.Log.LogError("GenerateLinkedDataTask: SourceFiles must not be empty.")
            false
        elif this.AssemblyRefs.Length = 0 then
            this.Log.LogError("GenerateLinkedDataTask: AssemblyRefs must not be empty.")
            false
        else
            this.RunGenerate()
    ...
    member private this.RunGenerate() =
        ...
        | Error msg ->
            this.Log.LogError($"GenerateLinkedDataTask: could not read lock file: {msg}")
            false
        | Ok lock ->
            ...
            | Error msg ->
                this.Log.LogError($"GenerateLinkedDataTask: FSI evaluation failed: {msg}")
                false
            | Ok registry ->
                match LinkedDataEmitter.emit this.ModuleName registry lock with
                | Error msg ->
                    this.Log.LogError($"GenerateLinkedDataTask: code generation failed: {msg}")
                    false
                | Ok source -> this.WriteOutput source

    member private this.WriteOutput(source: string) =
        let outPath = Path.Combine(this.OutputPath, "GeneratedLinkedData.fs")
        ...
        try
            ...
            true
        with ex ->
            this.Log.LogError($"GenerateLinkedDataTask: could not write '{outPath}': {ex.Message}")
            false
