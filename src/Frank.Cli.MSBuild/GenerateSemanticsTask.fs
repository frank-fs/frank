...
/// reconstruction, then calls SemanticModelEmitter to write GeneratedSemantics.fs to
/// the intermediate output directory. No child dotnet/msbuild processes are spawned.
type GenerateSemanticsTask() =
    inherit Task()
    ...
    override this.Execute() =
        if String.IsNullOrWhiteSpace this.LockFilePath then
            this.Log.LogError("GenerateSemanticsTask: LockFilePath must not be empty.")
            false
        elif String.IsNullOrWhiteSpace this.OutputPath then
            this.Log.LogError("GenerateSemanticsTask: OutputPath must not be empty.")
            false
        elif String.IsNullOrWhiteSpace this.ModuleName then
            this.Log.LogError("GenerateSemanticsTask: ModuleName must not be empty.")
            false
        elif this.SourceFiles.Length = 0 then
            this.Log.LogError("GenerateSemanticsTask: SourceFiles must not be empty.")
            false
        elif this.AssemblyRefs.Length = 0 then
            this.Log.LogError("GenerateSemanticsTask: AssemblyRefs must not be empty.")
            false
        else
            this.RunGenerate()
    ...
    member private this.RunGenerate() =
        ...
        | Error msg ->
            this.Log.LogError($"GenerateSemanticsTask: could not read lock file: {msg}")
            false
        | Ok lock ->
            ...
            | Error msg ->
                this.Log.LogError($"GenerateSemanticsTask: FCS evaluation failed: {msg}")
                false
            | Ok registry ->
                match SemanticModelEmitter.emit this.ModuleName registry lock with
                | Error msg ->
                    this.Log.LogError($"GenerateSemanticsTask: code generation failed: {msg}")
                    false
                | Ok source -> this.WriteOutput source

    member private this.WriteOutput(source: string) =
        let outPath = Path.Combine(this.OutputPath, "GeneratedSemantics.fs")
        ...
        try
            ...
            true
        with ex ->
            this.Log.LogError($"GenerateSemanticsTask: could not write '{outPath}': {ex.Message}")
            false
