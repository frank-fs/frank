...
module EndpointAuth =
    let private toConvention (requirement: AuthRequirement) : EndpointBuilder -> unit =
        ...
        | AuthRequirement.Authenticated ->
            fun b -> b.Metadata.Add(AuthorizeAttribute())
        ...
        | AuthRequirement.Role name ->
            fun b ->
                ...
                let policy =
                    ...
                    pb.Build()
                b.Metadata.Add(policy)
        | AuthRequirement.Policy name ->
            fun b -> b.Metadata.Add(AuthorizeAttribute(name))
    ...
    let applyAuth (config: AuthConfig) (spec: ResourceSpec) : ResourceSpec =
        if AuthConfig.isEmpty config then
            ...
            config.Requirements
            |> List.fold (fun s req ->
                ResourceBuilder.AddMetadata(s, toConvention req)) spec
