...
module ResourceBuilderExtensions =
    type ResourceBuilder with
        ...
        member _.RequireAuth(spec: ResourceSpec) : ResourceSpec =
            let config =
                AuthConfig.empty |> AuthConfig.addRequirement AuthRequirement.Authenticated
        ...
        member _.RequireClaim(spec: ResourceSpec, claimType: string, claimValue: string) : ResourceSpec =
            let config =
                AuthConfig.empty
                |> AuthConfig.addRequirement (AuthRequirement.Claim(claimType, [ claimValue ]))
        ...
        member _.RequireClaim(spec: ResourceSpec, claimType: string, claimValues: string list) : ResourceSpec =
            let config =
                AuthConfig.empty
                |> AuthConfig.addRequirement (AuthRequirement.Claim(claimType, claimValues))
        ...
        member _.RequireRole(spec: ResourceSpec, role: string) : ResourceSpec =
            let config =
                AuthConfig.empty |> AuthConfig.addRequirement (AuthRequirement.Role role)
        ...
        member _.RequirePolicy(spec: ResourceSpec, policyName: string) : ResourceSpec =
            let config =
                AuthConfig.empty
                |> AuthConfig.addRequirement (AuthRequirement.Policy policyName)
