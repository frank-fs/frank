...
module ProvenanceMiddleware =
    ...
    let tryMatchProvenanceAccept (acceptHeader: string) : string voption =
        if isNull acceptHeader || not (acceptHeader.Contains(ProvenanceMediaTypes.Prefix)) then
            ...
            match types |> List.tryFind (fun t -> acceptHeader.Contains(t)) with
            | Some t -> ValueSome t
            | None -> ValueNone
    ...
    let private standardMediaType (mediaType: string) =
        ...
        | unknown -> invalidOp $"Unrecognized provenance media type: %s{unknown}"
    ...
    /// The middleware request delegate factory.
    /// Logger is captured once at middleware construction time.
    let createProvenanceMiddleware (loggerFactory: ILoggerFactory) (next: RequestDelegate) =
        let logger = loggerFactory.CreateLogger("Frank.Provenance.Middleware")
