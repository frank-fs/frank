...
module JsonLdFormatter =
    ...
    let private localName (uri: string) =
        let lastHash = uri.LastIndexOf('#')
        let lastSlash = uri.LastIndexOf('/')
        let idx = max lastHash lastSlash
        if idx >= 0 && idx < uri.Length - 1 then uri.Substring(idx + 1) else uri

    let private namespaceUri (uri: string) =
        let lastHash = uri.LastIndexOf('#')
        let lastSlash = uri.LastIndexOf('/')
        let idx = max lastHash lastSlash
            ...
            for triple in triples do
                ...
                | :? ILiteralNode as lit ->
                    if isNull lit.DataType then
                        ...
                        if dtUri.EndsWith("integer") || dtUri.EndsWith("int") then
                            ...
                            match System.Double.TryParse(lit.Value, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture) with
                            | true, v -> writer.WriteNumber(predName, v)
                            | _ -> writer.WriteString(predName, lit.Value)
                ...
                | :? IUriNode as u ->
                    ...
                    writer.WriteEndObject()
                | _ ->
                    writer.WriteString(predName, triple.Object.ToString())
        | _ ->
