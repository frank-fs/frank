...
module Rdf =
    ...
    /// absolute IRI if it looks genuinely absolute -- it contains "://", or starts with an allow-listed
    /// non-hierarchical scheme ("urn:", "mailto:", "tel:") -- *and* is well-formed under
    /// System.Uri.IsWellFormedUriString. Anything else raises, including strings that
    /// System.Uri.IsWellFormedUriString alone would call well-formed (almost any "word:word" string
    /// qualifies under its loose absolute-URI rules, which is why that check alone isn't enough to
    /// catch a typo'd, undeclared CURIE prefix like "foaf:name"). Raises if there's no colon at all.
