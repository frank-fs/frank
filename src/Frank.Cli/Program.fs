...
let main args =
    ...
    let compileProjectOptionsFileOpt = Option<string>("--project-options-file")

    compileProjectOptionsFileOpt.Description <-
        "Path to JSON file with pre-resolved MSBuild project options (source files, references, defines). Generated by the MSBuild target to avoid redundant subprocess spawning."
    ...
    compileCmd.Options.Add(compileProjectOptionsFileOpt)
    ...
    compileCmd.SetAction(fun parseResult ->
        ...
        let projectOptionsFile = parseResult.GetValue(compileProjectOptionsFileOpt)
