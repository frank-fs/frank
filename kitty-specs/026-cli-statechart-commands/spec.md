...
Currently, these library functions have no user-facing entry point. A developer must write custom F# code to call generators, invoke parsers, or run validation. This feature restructures frank-cli into two subcommand groups — `frank-cli semantic` for the existing linked-data pipeline and `frank-cli statechart` for statechart pipeline operations — enabling both human developers and LLM coding agents to extract, generate, validate, and parse statechart artifacts without writing code. The `status` and `help` commands remain top-level utilities.

All parsers return `Ast.ParseResult` directly (no mapper modules). All generators consume `StatechartDocument` directly. The validation pipeline (spec 025) provides `Pipeline.validateSources` for end-to-end parse-and-validate orchestration.
...
### User Story 4 - Parse a Spec File for LLM-Assisted Scaffolding (Priority: P2)

A developer or LLM agent has a spec file (e.g., an XState JSON definition, a WSD diagram, or an smcat notation) and wants to produce a structured representation suitable for generating F# code. They run `frank statechart parse game.xstate.json` and receive a JSON representation of the parsed state machine -- states, transitions, guards, events -- that an LLM can consume to scaffold idiomatic F# state DUs, transition functions, guard stubs, and handler wiring.

**Why this priority**: Parse enables the "design-first" workflow where a developer sketches a state machine in a notation format and then uses LLM tools to generate the corresponding F# code. This is valuable but secondary to the "code-first" workflow (extract/generate/validate) which works with existing code.

**Independent Test**: Create a simple XState JSON file with three states and two transitions, run `frank statechart parse`, and verify the output JSON contains the expected state names, transition events, and structure.

**Acceptance Scenarios**:

1. **Given** a valid XState JSON file, **When** `frank statechart parse game.xstate.json` is invoked, **Then** the output is a JSON representation of the parsed `StatechartDocument` containing states, transitions, and metadata.
2. **Given** a valid WSD file, **When** `frank statechart parse onboarding.wsd` is invoked, **Then** the output contains the parsed state machine in JSON `StatechartDocument` format.
3. **Given** a valid smcat file, **When** `frank statechart parse game.smcat` is invoked, **Then** the output contains the parsed state machine in JSON `StatechartDocument` format.
4. **Given** a valid ALPS JSON file, **When** `frank statechart parse game.alps.json` is invoked, **Then** the output contains the parsed state machine in JSON `StatechartDocument` format.
5. **Given** a valid SCXML file, **When** `frank statechart parse game.scxml` is invoked, **Then** the output contains the parsed state machine in JSON `StatechartDocument` format.
6. **Given** a spec file with parse errors, **When** `frank statechart parse broken.wsd` is invoked, **Then** the output includes the best-effort parsed document along with the parse errors and warnings, and the command exits with a non-zero exit code.
7. **Given** `frank statechart parse game.xstate.json --output-format json`, **When** the output is parsed, **Then** it matches the shared AST `StatechartDocument` schema with states, transitions, events, guards, and annotations.
...
- State names containing special characters (spaces, hyphens, Unicode) are correctly preserved through extract/generate/parse roundtrips
...
- The parse command detects file format from the file extension; ambiguous extensions (e.g., `.json` which could be ALPS or XState) are resolved by attempting both parsers and reporting which succeeded, or by requiring an explicit `--format` flag for notation disambiguation
...
- **FR-018**: System MUST provide deserialization of XState v5 JSON to produce an `Ast.ParseResult` (containing a `StatechartDocument`, errors, and warnings) for use by the parse command
...
- **FR-021**: System MUST generate a `FormatArtifact` from the extracted `StateMachineMetadata` (by running it through the WSD generator to produce a `StatechartDocument`) to serve as the "code truth" artifact for cross-format validation
...
- **FR-026**: System MUST provide a `frank statechart parse <spec-file>` command that parses the spec file and outputs the resulting `StatechartDocument` as JSON
- **FR-027**: System MUST detect the spec file format from the file extension (same mapping as FR-020)
- **FR-028**: System MUST delegate to the appropriate format parser: WSD parser (spec 007), ALPS JSON parser (spec 011), SCXML parser (spec 018), smcat parser (spec 013), or XState JSON deserializer (FR-018). All parsers return `Ast.ParseResult` directly (no mapper step).
...
- **FR-032**: All commands MUST support `--output-format text|json` for output rendering, defaulting to `text`. The `generate` and `parse` commands additionally use `--format` for notation format selection (wsd/alps/scxml/smcat/xstate/all on generate; wsd/alps/scxml/smcat/xstate on parse for ambiguous file extension disambiguation). The principle: `--format` = what notation format, `--output-format` = how to render output.
- **FR-033**: JSON output MUST follow the same conventions as existing frank-cli JSON output (from `JsonOutput` module)
- **FR-034**: Text output MUST respect the existing `NO_COLOR` environment variable convention (as implemented in the `TextOutput` module)

**CLI Structure and Integration**

- **FR-035**: All statechart commands MUST be registered under a `statechart` parent command on the root command, creating the `frank-cli statechart <subcommand>` structure
- **FR-036**: Existing linked-data pipeline commands (extract, clarify, validate, diff, compile) MUST be moved under a `semantic` parent command, creating the `frank-cli semantic <subcommand>` structure. The `status` and `help` commands remain top-level utilities.
- **FR-037**: System MUST integrate with the existing `frank-cli` System.CommandLine infrastructure in `Program.fs`, restructuring the command hierarchy into `semantic` and `statechart` parent commands with their respective subcommands
- **FR-038**: System MUST register help metadata for each new and restructured command following the patterns established by spec 016 (help system), including summary, examples, workflow position, and context description. Help lookup MUST support hierarchical names (e.g., `frank-cli help statechart extract` and `frank-cli help semantic extract` as distinct lookups).
- **FR-039**: System MUST add a `statechart-workflows` help topic describing the statechart pipeline (extract → generate → validate → parse) and update the existing `workflows` topic to be namespaced as `semantic-workflows`
...
- **FormatDetector**: Logic for mapping file extensions to format tags (`.wsd` -> WSD, `.alps.json` -> ALPS, `.scxml` -> SCXML, `.smcat` -> smcat, `.xstate.json` -> XState JSON, `.json` -> ambiguous). Used by validate and parse commands to determine which parser to invoke.
...
- **SC-004**: The parse command successfully parses files in all five formats and produces a `StatechartDocument` JSON representation that preserves all states, transitions, events, and guards from the source file
...
- **SC-007**: Existing frank-cli commands (extract, clarify, validate, diff, compile — now under `semantic`; plus top-level status and help) continue to function correctly with no regressions after the CLI restructuring
...
- All commands use `--output-format` for text/json output rendering. The `--format` flag is used for notation format selection: on the `generate` command for target notation (wsd/alps/scxml/smcat/xstate/all), and on the `parse` command for notation disambiguation of ambiguous file extensions (e.g., `.json`).
- File extension detection for format identification is sufficient for all practical cases. The `.json` ambiguity (ALPS vs XState) is resolved by convention (`.alps.json` for ALPS, `.xstate.json` for XState JSON) or by explicit `--format` flag on the parse command.
