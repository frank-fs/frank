...
- **FR-002**: System MUST define a `StateNode` record type representing a state, containing: a unique string identifier, an optional display label, a `StateKind` discriminated union (Regular, Initial, Final, Parallel, ShallowHistory, DeepHistory, Choice, ForkJoin, Terminate), optional child `StateNode` records (for hierarchy), optional state activities (entry, exit, do actions), an optional source position, and a list of format-specific annotations
...
- **FR-019**: System MUST define a `HistoryKind` discriminated union with cases `Shallow` and `Deep`, used by `ScxmlMeta.ScxmlHistory` annotations to carry the SCXML `<history type="shallow|deep">` type attribute. Note: `StateKind` uses flat `ShallowHistory` and `DeepHistory` cases (see FR-005) rather than wrapping `HistoryKind`
...
- **StateNode**: A state with identifier, optional label, kind (Regular/Initial/Final/Parallel/ShallowHistory/DeepHistory/Choice/ForkJoin/Terminate), optional child states (for compound/parallel states), optional activities (entry/exit/do), optional source position, and annotations list.
