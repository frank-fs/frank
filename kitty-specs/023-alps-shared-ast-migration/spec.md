...
- **FR-010**: System MUST store pure data descriptors (semantic descriptors that are not states and not transitions) as `AlpsMeta` annotations on `StatechartDocument.Annotations` or as `DataEntry` records in `StatechartDocument.DataEntries`, preserving their `id` and optional documentation
