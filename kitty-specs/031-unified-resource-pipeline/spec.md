...
- Q: How is the affordance map loaded into the running application at runtime? → A: Embedded as binary via MSBuild target. The CLI generates `obj/frank-cli/unified-state.bin` (MessagePack binary); an MSBuild target auto-embeds it as `EmbeddedResource` with logical name `Frank.Affordances.unified-state.bin`. At startup, the middleware deserializes the binary via `Assembly.GetManifestResourceStream()` and projects all runtime views (affordance map lookup, ALPS, OWL/SHACL, JSON Schema) into memory. No resx — use `<EmbeddedResource>` directly.
...
- **FR-017**: System MUST provide a `useAffordances` custom operation on the `webHost` CE (same pattern as `useOpenApi`) that registers the affordance middleware, loads the embedded binary at startup, and projects all runtime views. The middleware injects affordance headers into responses for stateful resources
...
- **FR-033**: System MUST use `FSharp.Data.JsonSchema.OpenApi` as the canonical opinion for F# type-to-schema mapping across all projections: OpenAPI schemas, JSON Schema served via `rel="describedby"`, CLI JSON output, and affordance map display format
- **FR-034**: The affordance middleware MUST serve per-resource JSON Schema at dedicated URLs and include `Link: <schema-url>; rel="describedby"` headers in responses, complementing the ALPS `rel="profile"` link
...
- **affordance map** — the concept: a mapping of (resource, state) to available actions
- **`AffordanceMap`** — the F# record type representing the deserialized map
- **`--format affordance-map`** — the CLI flag for generating the map
- **`unified-state.bin`** — the binary file (MessagePack) containing the full `UnifiedExtractionState`, from which the affordance map and all other views are projected
- **projection** — computing a specific view (ALPS, OWL, SHACL, JSON Schema, affordance lookup) from the unified state at startup

## Success Criteria

### Measurable Outcomes

- **SC-001**: A developer can run `frank-cli extract --project <fsproj>` and receive a unified description of all resources (type + behavior + routes) in a single command, completing within 15 seconds wall-clock time (cold, no cache) for a project with up to 20 resources
...
- The statechart middleware stores the current state key in `HttpContext.Items` under a key to be verified against `src/Frank.Statecharts/Middleware.fs` during WP07 implementation — the WP07 prompt instructs the implementer to read the actual key from the middleware source rather than assuming a name
...
- Existing format generators (WSD, SCXML, smcat, XState) consume `StatechartDocument` AST, not `UnifiedResource` directly. The unified model's behavioral data is projected to `ExtractedStatechart` (via `StatechartExtractor.toExtractedStatechart`), then to `StatechartDocument` (via `FormatPipeline.buildDocumentFromExtracted`), then to format text via format-specific serializers. The unified extraction does not change the generators — only the extraction input path
