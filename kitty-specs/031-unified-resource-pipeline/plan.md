...
| IV. ASP.NET Core Native | **PASS** | Middleware uses standard `IApplicationBuilder` extension. Embedded resources via standard `GetManifestResourceStream`. Content negotiation via standard Accept header handling. |
...
│   ├── AffordanceMiddleware.fs     # UseAffordances() — Link + Allow headers
