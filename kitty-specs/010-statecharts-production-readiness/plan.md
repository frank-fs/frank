...
3. **Guard DU** (P2): Replace the current `Guard` record type with a discriminated union having two cases: `AccessControl` (receives user, state, context -- runs pre-handler, no event parameter) and `EventValidation` (receives user, state, event, context -- runs post-handler with actual event). The DU case determines both execution phase and type signature. No `'Event option`, no separate `EventGuards` field, no phase markers.
...
**Constraints**: No breaking changes to `IStateMachineStore` interface. Breaking change to `Guard` type (from record to DU) and `StateMachine` record (replacing `Guards` field type) is acceptable for pre-1.0 library. ETag computation (`StatechartETagProvider`) must NOT use case-name keys -- it correctly uses full `string state` for parameter-sensitive hashing.
...
| II. Idiomatic F# | PASS | Guard type becomes a discriminated union (idiomatic F# modeling of choices). DU-based state types, computation expressions for configuration, `FSharpValue.PreComputeUnionTagReader` is idiomatic F# reflection. |
...
├── plan.md              # This file
├── research.md          # Phase 0 output (complete, revision 3)
├── data-model.md        # Phase 1 output (complete, revision 3)
├── checklists/
...
│   ├── Types.fs                          # Guard DU, GuardContext split, StateMachine record update
│   ├── Store.fs                          # Documentation-only: document actor-serialization contract
│   ├── StatefulResourceBuilder.fs        # State key extraction, guard DU dispatch in closures
│   ├── Middleware.fs                     # Two-phase guard evaluation (pre-handler + post-handler)
...
│   ├── TypeTests.fs                      # Tests for Guard DU type
...
Foundation (WP01-WP02, sequential)
  ...
  WP02: Guard DU (Types.fs, StatefulResourceBuilder.fs, Middleware.fs) -- depends on WP01 for shared Types.fs changes
...
- **Sequential work (Foundation)**: WP01 (state key extraction) must land first because WP02 modifies the same `StateMachine` record. WP02 (Guard DU) modifies `Types.fs`, `StatefulResourceBuilder.fs`, and `Middleware.fs`.
...
- **After WP02**: Guard DU finalized in `Types.fs`; middleware flow updated with two-phase evaluation; `StateMachineMetadata` has `EvaluateEventGuards` field
...
Decisions are fully documented in [research.md](research.md) (revision 3). Key highlights:
...
| D-003 | Guard DU with `AccessControl` and `EventValidation` cases | Single DU type replaces `Guard` record. DU case determines execution phase AND type signature. No `'Event option`, no separate fields, no phase markers. Breaking change acceptable for pre-1.0. |
