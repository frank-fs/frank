...
- **Role-pruning edge case on semantic references:** the cross-document resolution fallback assumes the root document contains everything a filtered view might reference, but the full document itself can be role-pruned per principal (per `AuthorizationFilter`) — so a kept descriptor referencing a transition that got pruned out of that same document for that same principal will resolve to a URL fragment (`rootUri#thatId`) that doesn't actually exist in the response the caller just received. Narrow in practice (semantic/state references are never pruned, only transition-to-transition references can hit this), not fixed in this branch, worth a line of awareness for future work.
- **Analyzer guardrail**: at most one `useAlps` per `webHost` and at most one per resource. Real gap, separate issue — not blocking this design. `AlpsDocument.fs`'s `install` registers `AlpsOptions` as a DI singleton unconditionally on every `useAlps` call, so two `useAlps` calls in one host would silently make the last-registered one win for `Excerpt.fs`'s cross-document resolution, independent of which document's route the request actually hit.
