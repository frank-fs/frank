...
Mechanically: whatever Frank core's content-negotiation mechanism turns out to be (see #482 below — genuinely open, not settled), the shape is a representation-selector keyed by `Accept` that, when JSON-LD is selected, builds a `Doc` from whatever the handler passed and writes it via `Doc.writeJsonLd` directly into the response stream — no string in between, composing with the writer-based path from *Serialization*. That selector/formatter is app-level (or a small separate future package, the way `Frank.OpenApi`/`Frank.JsonHome` sit next to `Frank` core rather than inside it) — not something `Frank.Rdf` itself should implement, since doing so would need ASP.NET Core HTTP references and break this package's zero-dependency boundary (see *Package shape*). `Frank.Rdf` still has no opinion on response plumbing or on how Frank core ends up doing content negotiation; it just now clearly hands off to content negotiation rather than a sibling resource, and *that* choice needed to be a documented decision, not an implicit gap.

**Two prerequisites, tracked separately, before any of this gets built for real:**

- [frank-fs/frank#481](https://github.com/frank-fs/frank/issues/481) — there is no shared per-resource Link-advertisement mechanism today. `Frank.JsonHome` and `Frank.OpenApi` each rolled their own independent, near-identical `Response.OnStarting`/`Headers.Append("Link", ...)` middleware rather than sharing the `IResponseLinkProvider` abstraction the original JsonHome design doc proposed — and both existing uses are app-wide singletons, not resource-scoped, which doesn't fit a per-game Link the way this design would need.
- [frank-fs/frank#482](https://github.com/frank-fs/frank/issues/482) — bigger than a testing gap. `ContentNegotiation.fs`'s `IOutputFormatter`-based approach is itself, by its own source's own description, "content negotiation *by hand*" — a workaround reaching into MVC's `ObjectResultExecutor` internals, because ASP.NET Core (unlike the older Web API) never shipped content negotiation as a standalone service. `ctx.Negotiate` is also never actually called anywhere in the codebase, and the one sample claiming to demonstrate it (`getProductNegotiated`, `Frank.OpenApi.Sample`) is fake — its own comment admits it always returns JSON regardless of `Accept`. The issue calls for evaluating a lean, Frank-native alternative built on `Microsoft.Net.Http.Headers.MediaTypeHeaderValue` (genuinely standalone, not MVC-coupled) before assuming the existing `IOutputFormatter` approach is the mechanism to build on — a formatter for `application/ld+json` would be the first real exercise of whichever mechanism is chosen, not one more use of a proven one.

Given that, this section describes the target design, not something ready to build immediately — the tic-tac-toe follow-on plan (and any Frank-core work toward per-resource JSON-LD serving) should treat #481 and #482 as blocking, not as background reading.
