...
Mechanically: Frank's `ContentNegotiation.fs`/`ctx.Negotiate(statusCode, body)` already selects among registered ASP.NET Core MVC `IOutputFormatter`s and is the natural place for this — a formatter registered for `application/ld+json` that, when selected, builds a `Doc` from whatever `body` the handler passed and writes it via `Doc.writeJsonLd` directly into the response stream (`OutputFormatterWriteContext` hands a `Stream`, so this composes with the writer-based path from *Serialization* — no string in between). That formatter is app-level (or a small separate future package, the way `Frank.OpenApi`/`Frank.JsonHome` sit next to `Frank` core rather than inside it) — not something `Frank.Rdf` itself should implement, since doing so would need `Microsoft.AspNetCore.Mvc.Formatters`/`Microsoft.AspNetCore.Http` references and break this package's zero-ASP.NET-Core-dependency boundary (see *Package shape*). `Frank.Rdf` still has no opinion on response plumbing; it just now clearly hands off to content negotiation rather than a sibling resource, and *that* choice needed to be a documented decision, not an implicit gap.
