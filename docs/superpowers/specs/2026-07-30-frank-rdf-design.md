...
| Situation | Behaviour |
|---|---|
| A `propertyString`/`propertyInt`/etc., `typ`, or predicate CURIE uses an undeclared prefix | Throws unless the string also *looks* genuinely absolute — contains `://`, or starts with an allow-listed non-hierarchical scheme (`urn:`, `mailto:`, `tel:`) — *and* is well-formed per `Uri.IsWellFormedUriString`. A syntactically URI-shaped typo like `foaf:name` with no declared `foaf` prefix no longer satisfies the `looksAbsolute` gate (no `://`, no allow-listed scheme), so it now raises `Frank.Rdf: undeclared prefix 'foaf' in 'foaf:name'` instead of silently passing through as the literal (wrong) IRI `<foaf:name>`. Legitimate absolute IRIs (`http://...`, `https://...`, `urn:...`, `mailto:...`) still pass through unchanged. Fixed by [frank-fs/frank#484](https://github.com/frank-fs/frank/issues/484). |
