...
### Serving it over HTTP: content negotiation on the resource itself

Not decided anywhere earlier in this design, and worth being explicit about now rather than leaving it implicit: JSON-LD is a **representation choice on the resource itself** — `GET /games/{id}` with `Accept: application/ld+json` returns JSON-LD, `Accept: text/html` returns the HTML page, `Accept: application/json` returns plain JSON, all at the *same* URL. Not a separate sibling resource.

This follows the `MediaTypeFormatter`/`IOutputFormatter` tradition from ASP.NET's OWIN/Web API lineage: a formatter's job is "given this media type won, produce a body" — never restricted to different serializations of one shared object graph. Early Web API-era formatters included, for instance, image representations built from a per-resource-id database lookup, sharing nothing with whatever formatter produced the JSON representation of the same resource. HTML, JSON, and JSON-LD for a game are exactly this: three different representation-generation paths for the same resource, correctly selected by `Accept`, not three different resources.

`JsonHome`/`ALPS`'s `Link`-header pattern is not the relevant precedent here and an earlier draft of this section wrongly reached for it — those are singleton, application-level discovery documents (there's one home document, one ALPS profile for the whole app), which is a different problem from choosing a representation of one specific resource instance. (If ALPS ever became a *per-resource* profile rather than application-wide, it would plausibly join this discussion — not the case today.)

Mechanically, both prerequisites this section originally deferred to have since landed in Frank core:

- **Resource-scoped `Link` advertisement** — [frank-fs/frank#481](https://github.com/frank-fs/frank/issues/481), design doc `docs/superpowers/specs/2026-07-31-frank-link-provider-design.md`. `resource { }` gained a `link` custom operation (`src/Frank/WebLink.fs`, `src/Frank/ResourceBuilder.fs`): `link target rel` for a static entry, or `link (fun ctx -> WebLink seq)` for a per-request provider. It writes into that endpoint's own metadata (an `internal ResourceLinkProvider`, read back by `WebLink.useResourceScopedLinks`) rather than an app-wide list, so a `Link: </games/1>; rel="alternate"; type="application/ld+json"` entry can be scoped to one game's resource — exactly the gap this design originally flagged. `Frank.JsonHome`/`Frank.OpenApi` were migrated onto the same shared `WebLink` type for their existing app-wide entries; nothing about their behavior changed.
- **Frank-native content negotiation** — [frank-fs/frank#482](https://github.com/frank-fs/frank/issues/482). Rather than proving out the existing `IOutputFormatter`-based `ContentNegotiation.fs`, Frank core added an independent `negotiate { }` CE (`src/Frank/NegotiateBuilder.fs`) built on `Microsoft.Net.Http.Headers.MediaTypeHeaderValue` — the lean, MVC-decoupled alternative #482 called for. Its `accepts mediaType handler` custom operation registers one representation per media type as a genuinely independent code path (not one object serialized differently per formatter), which matches this design's actual need. `negotiate { }`'s `Run` returns an ordinary `HandlerDefinition`, so it composes with `resource { }`'s `get`/`post` exactly the way `handler { }` does: `get (negotiate { accepts "application/ld+json" gameJsonLdHandler; accepts "text/html" gameHtmlHandler })`. (`ContentNegotiation.fs`'s older `IOutputFormatter`-based `negotiate`/`ctx.Negotiate` still exists as a separate mechanism, kept as a bridge rather than deleted; the two share the `negotiate` identifier and shadow by `open` order — see `NegotiateBuilder.fsi`'s own doc comment before using both in one file.)

With both pieces in place, the tic-tac-toe follow-on plan can build the actual `GET /games/{id}` handler: a `negotiate { }` block with an `application/ld+json` representation that builds a `Doc` from `gameLinkedData` and writes it via `Doc.writeJsonLd` directly into the response stream (composing with the writer-based path from *Serialization*), alongside the existing HTML/JSON representations, plus a resource-scoped `link` entry advertising the JSON-LD representation. That wiring stays `Frank.Rdf`-external — this package still has no ASP.NET Core dependency and no opinion on response plumbing (see *Package shape*) — but it is no longer blocked on anything in Frank core.

## Error handling and edge cases

| Situation | Behaviour |
|---|---|
| A `propertyString`/`propertyInt`/etc., `typ`, or predicate CURIE uses an undeclared prefix | Throws unless the string also *looks* genuinely absolute — the text immediately after the parsed scheme's colon starts with `//` (anchored there, not merely `://` appearing anywhere later in the string), or the string starts with an allow-listed non-hierarchical scheme (`urn:`, `mailto:`, `tel:`, matched case-insensitively per RFC 3986 §3.1) — *and* is well-formed per `Uri.IsWellFormedUriString`. A syntactically URI-shaped typo like `foaf:name` with no declared `foaf` prefix no longer satisfies the `looksAbsolute` gate (nothing after its colon starts with `//`, no allow-listed scheme), so it now raises `Frank.Rdf: undeclared prefix 'foaf' in 'foaf:name'` instead of silently passing through as the literal (wrong) IRI `<foaf:name>`. The anchoring also catches a stray leftover prefix like `schema:http://weird` (undeclared `schema`, `://` present but not immediately after the colon), which an unanchored `Contains "://"` check would have let through. Legitimate absolute IRIs (`http://...`, `https://...`, `urn:...`, `mailto:...`, `URN:...`, in any case) still pass through unchanged. Fixed by [frank-fs/frank#484](https://github.com/frank-fs/frank/issues/484). |
