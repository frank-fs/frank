...
| `Position` field | a **domain-vocabulary** term `ttt:square` under a declared `prefix "ttt"` | **There is no honest schema.org term for a tic-tac-toe square** — `schema:position` means *ordinal position in a series*, not a board cell. This is a real finding, not a gap to paper over: the thesis is *reuse global vocabulary where it fits (agent), domain vocabulary where it must (square)*. **Original-blocker resolution (verified 2026-06-28):** `accept` cannot today resolve a declared-but-unfetched prefix — `prefixes` is built only from `lf.Vocabularies` (fetched vocabs) at `Accept.fs:391`, so `ttt:square` hits "Unknown prefix 'ttt'" *before* the fail-open path (`Accept.fs:302-303`) is ever reached. Minimal capstone fix: carry declared `prefix` namespaces (registry `Prefixes`) into accept so the CURIE **expands** to its absolute IRI for codegen; then, because `ttt` has no fetched graph / no oracle coverage, `accept` **warns** ("vocabulary `ttt` referenced but not published — host it") and still writes the mapping — it does NOT hard-reject. Genuine typos in a *covered* vocab still reject. The author publishes `ttt` by hosting it as a normal resource (D-deref). |

> **Mapping ≠ vocabulary authoring (clarified 2026-06-28).** The CLI `extract→clarify→accept` flow maps F#
> types to *existing* IRIs; it does not author or host the vocabulary. Authoring + hosting the `ttt` ontology
> is a separate concern, done by the developer (LLM-assisted) as a normal resource (D-deref). `accept` only
> *warns* when a referenced vocab is unpublished — it never authors one.
...
1. **The sample SERVES its domain (`ttt:`) vocabulary — as a normal Frank resource.** Since no global term
   fits a board cell, the sample acts as a Linked Data *publisher*. A custom vocabulary is **just another
   resource** (Constitution pillar 1): the developer (LLM-assisted) authors the `ttt` ontology document and
   exposes it at a route, content-negotiated (turtle/ld+json), so `ttt:square` dereferences to a definition.
   No special publish/`useVocabulary` mechanism — `useLinkedData`'s serializers may be reused, but the vocab
   is hosted like any resource. The lock IRI base is build-time-fixed while the test host is dynamic
   (localhost:port) — the agent-simulator **rebases the term's path onto the test server base it is already
   talking to**. `ttt` is a **hash namespace** (`…tictactoe#square`) — deref strips the fragment and GETs the
   one doc, so hosting the whole vocab at one base path is the correct deref. The spike must prove a GET on
   the (rebased) `ttt:square` IRI resolves (2xx) to something describing the term.
...
- **First-class "unpublished vocabulary" guidance — two separate streams (own issues, NOT this capstone):**
  - **Stream 2** — a `frank semantic` **design-time warning** when a referenced vocabulary is not published
    (not dereferenceable / not hosted), guiding the author to host it. The CLI runs without the app's routes
    in view, so it checks dereferenceability / the lock's fetch record only.
  - **Stream 3** — a **Frank.Analyzers compile-time analyzer** that warns when a referenced vocabulary URL
    cannot be dereferenced (per the lock's fetch record — no live network in the analyzer) AND is not routed
    in the current app. Warn-level; error only under `WarningsAsErrors`.

  The capstone includes ONLY the *minimal* `accept` warn-not-reject (D2) needed for the honest mapping to
  land. The broader CLI guidance (Stream 2) and the analyzer (Stream 3) are designed + expert-reviewed
  (TBL/Miller) as their own increments.
