...
- OpenAPI documentation for representations sharing a status code but declaring *different* response types (e.g. one `handler{}`-wrapped representation returning `Product`, another returning some other type, both at `200`). Implementation (see `docs/superpowers/plans/2026-07-31-content-negotiation.md`, Task 8) merges same-`(status code, response type)` metadata into the OpenAPI document correctly; representations that share a status code with genuinely different types are left as separate metadata objects, and `Microsoft.AspNetCore.OpenApi`'s own last-registered-wins behavior still silently drops all but one from the generated document. Fixing this fully would need a custom `IOpenApiOperationTransformer` in `Frank.OpenApi` that synthesizes per-content-type schemas — not attempted, since the common case (same type, several content types) already works via the metadata merge.
