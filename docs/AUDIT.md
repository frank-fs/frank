...
The most likely explanation: the merge was *mostly successful* while flat was still the correct model. The flat types in `Frank.Resources.Model` (`ExtractedStatechart`, `TransitionSpec`, `StateContainment`) are the successfully migrated flat versions — they worked correctly for the flat use case. But the cleanup step was missed: `Frank.Statecharts.Core` wasn't deleted. When hierarchy work started later, Claude saw two assemblies with AST-like types and put the hierarchical enhancements (`StateKind.Parallel`, `StateNode.Children`, history pseudo-states, `TransitionAlgebra`) into Core — the assembly that should have been gone. The flat types in `Frank.Resources.Model` never got the hierarchical enhancements because those landed in the wrong assembly.

The "circular dependency" comment in `ResourceTypes.fs` is the evidence — by the time anyone tried to reconcile the two, they'd diverged enough that merging would have required resolving the dependency direction. The resolution was to keep both and bridge between them. The assembly that should have been deleted became the one that 32 files actually import from, and the assembly that should hold the shared AST still has the flat versions from the original successful merge.
...
The result is that the "portable concept" — carrying hierarchy, roles with role-based projections, guards, ALPS semantics, SHACL constraints, and provenance hooks through the whole system — doesn't exist as a single coherent type. `ExtractedStatechart` was supposed to be it, but it's flat and missing half the concerns. `StateContainment` exists separately but isn't on `ExtractedStatechart`. SHACL shapes are served but the affordance middleware doesn't know about them. ALPS annotations live on AST nodes but don't flow into the resource model. Provenance has its own event type that doesn't match the statechart's event type. The discovery middleware and the affordance middleware are separate systems with separate data paths.
