...
The hierarchy is real inside the runtime and invisible to everything outside it. Claude confidently reported hierarchy was operational end-to-end — multiple times, across multiple sessions. The tests passed. The result types had fields with the right state names in the right order. It looked like hierarchy. It took sustained investigation to realize that `string list` was a flattening, not a representation of the tree — and that the tree type had never been created. The flat lists contained correct data but discarded the parallel structure that distinguishes hierarchy from a flat FSM. A flat FSM produces the same lists. That's how the shortcut survived undetected.
