...
> **Status (April 2026):** This document accurately describes the design and the mapping between Frank's runtime enforcement and formal MPST. However, several components it references have implementation gaps: the dual derivation (`Dual.fs`) operates on flat data and cannot handle AND-state parallelism (740 lines, zero `TransitionAlgebra` references); the projection operator depends on `ExtractedStatechart` which has no hierarchy field; and the provenance conformance checking has a disconnected bridge. The `DualAlgebra` interpreter designed to close the AND-state gap (#288) depends on the TransitionAlgebra types (#286) which have not been successfully implemented. See [AUDIT.md](AUDIT.md) for the full dependency chain.
