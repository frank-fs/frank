...
Every new `src/Frank.*` package's implementation plan must include, in the SAME plan (not deferred to a follow-on), a `README.md` in the package directory and a runnable sample under `sample/` that demonstrates it working — not just unit tests. Unit tests plus a design doc do not prove a package works as real, runnable software; a sample does. `hooks/check-new-package-deliverables.sh` (a `Stop` hook) flags packages introduced on the current branch that are missing either, as a reminder — it does not block, so plan for these up front rather than relying on it to catch the gap.
