...
- **Docs/walkthrough narrated not run** — a README/walkthrough is false-green when its commands are described, not executed. Verify by running EVERY command in order as a fresh reader (fresh cwd, no prior state) and asserting the doc's pasted output matches a real capture. Caught #335: an accept-output line copy-pasted from the wrong scenario, and a "valid POST → 200" body that actually returned 400. Docs get the same "follow it literally" rigor as code.
- **Non-runnable harness** — "N tests pass" is hollow if the suite can't start (build gate, fixture misconfig, missing flag). A TDD test that can't execute in a clean run proves nothing. Verify the suite RUNS from your own invocation and report the count you observed — not that code merely compiles. Caught #335: E2E was 0/13 (ServerFixture didn't apply the NU1903 `TreatWarningsAsErrors=false` workaround) while the author claimed "13 pass."
...
- **Shell `curl`/`nc` are sandboxed OUT; .NET `HttpClient` is sandboxed IN.** Probing live-network reachability with shell tools gives a false "no egress" — this misled ~5 agents into reporting the live-schema.org-deref test (AT-S6) as "environmental" when it would actually pass. Probe egress the way the test does: `dotnet fsi` GET (via a `.fsx` script file — `dotnet fsi -e` is unsupported here), not shell `curl`. **schema.org egress is INTERMITTENT** — it flaps up/down within a single session, so a live-network test is flaky, not binary-blocked, and one probe isn't definitive; a momentary outage is not "environmental failure." Prefer offline loopback stubs (`HttpListener`, see #334 `RefreshCliTests`; #380) over live dereference.
- **CLI diff/drift exit codes: 2 = differences-found, 1 = error, 0 = none.** Follow git `--exit-code` / terraform `-detailed-exitcode` convention; never collapse "differences detected" into the error code. Don't over-adhere to a loose AC that conflicts with this — question the AC. Caught late in #334: directing drift→exit-1 to match an AC's literal "exit 1" collided drift with operational errors; reverted to 2.
