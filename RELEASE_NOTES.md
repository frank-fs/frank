...
- **Undeclared-prefix fallback tightened** ([#484](https://github.com/frank-fs/frank/issues/484)): passing through an undeclared prefix as an absolute IRI now additionally requires the string to *look* genuinely absolute — it must contain `://`, or start with an allow-listed non-hierarchical scheme (`urn:`, `mailto:`, `tel:`) — before `Uri.IsWellFormedUriString` is even consulted. Previously, any `word:word`-shaped string satisfied .NET's lax well-formedness rules regardless of scheme, so a typo like `foaf:name` with no `foaf` prefix declared silently became the literal (wrong) IRI `<foaf:name>` instead of raising. This is a deliberate behavior tightening: such typos now raise `Frank.Rdf: undeclared prefix '...' in '...'` instead of silently passing through. Legitimate absolute-IRI passthrough (`http://...`, `https://...`, and the allow-listed schemes above) is unaffected.
...
- **State-based filtering, with no store dependency:** `from [ states ]` marks a transition valid only from the given source state(s), and `CurrentStateResolver` (a plain `string -> Uri option` wired at composition time) answers what state a specific resource is in. The natural implementation queries a provenance or event store; an absent resolver, or one returning `None`, simply means state filtering does not apply. Matching walks `contains` ancestry rather than requiring exact equality, so being in a substate satisfies a transition declared `from` any of its ancestors — the correct reading of Harel state-configuration semantics. A transition with no `from` is never state-filtered.
