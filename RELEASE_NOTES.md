...
- **Breaking change:** `Serialization.toJson` (public) gained a leading `rootUri: Uri` parameter — `profile: Descriptor list -> string` is now `rootUri: Uri -> profile: Descriptor list -> string`. This also changes the wire format for any served document that's a filtered view of the full profile (a per-resource excerpt, or a role-pruned document): a cross-document reference to a descriptor absent from that filtered view now resolves to `rootUri#id` instead of a dangling `#id` fragment.
- **Breaking change: `ProtocolTransition` shape change for compound (orthogonal/history) transitions.** `{ FromState: Descriptor; Transition: Descriptor; ToState: Descriptor }` is now `{ FromGuard: StateGuard option; Transition: Descriptor; ToTargets: TransitionTarget list }`. Two deliberate behavior changes ride along with the shape change:
