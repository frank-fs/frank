...
- **Sample:** `sample/Frank.Alps.Sample` demonstrates both exposures on a tic-tac-toe game resource — the app-wide document at `/.well-known/alps.json` and the per-resource excerpt negotiated at `/games/{id}` — each advertised by its own `Link` header (app-wide and resource-scoped respectively). The sample also demonstrates a second, linked ping/pong protocol proving cross-document reference resolution, state-gated turns (with real 409 enforcement on a wrong-turn call), and role-projected views of the full document. A third protocol, a signaled `/intersections` resource, demonstrates compound transitions over real HTTP: two orthogonal regions (vehicle/pedestrian signals as simultaneously-active states), `walk` gated by a structural `All` guard spanning both regions (enforced server-side, with a real 409 once the guard no longer holds), an unconditional `emergencyOverride` fan-out entering both regions' flashing state at once, and `emergencyClear` restoring each region's actual prior substate via `History` — not a hardcoded reset to the initial state. `emergencyOverride`/`emergencyClear` are `requireRole "operator"`-gated (reusing ping/pong's `PingPongAuth` scheme), while `walk` and the plain intersection views stay unauthenticated — proving `StateGuard`-based filtering and role-based authorization filtering compose on the same document, not just each in isolation elsewhere in the sample.
