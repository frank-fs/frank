...
- **CURIE resolution is declared-prefix-first:** a string is resolved against a declared `prefix` before falling back to "is this already a well-formed absolute URI" — the other order is a real bug, not a style choice, since `.NET`'s `Uri` parser accepts unrecognized schemes (`"schema:Game"` parses as well-formed with scheme `"schema"`), which would otherwise let the single most common kind of CURIE this CE exists to construct silently pass through unresolved. This ordering fixes that specific bug — a declared prefix always wins over accidental well-formedness — but it is not a general fail-fast guarantee for undeclared or mistyped prefixes: a typo like `foaf:name` with no `foaf` prefix declared is itself a well-formed absolute URI by .NET's lax definition, so most such typos currently pass through silently as a wrong literal IRI rather than raising. See [frank-fs/frank#484](https://github.com/frank-fs/frank/issues/484), which tracks tightening this.
...
- **No breaking changes:** entirely additive, new package. No sample project yet -- HTTP-touching sample work is follow-on.
...
- **No Breaking Changes:** entirely additive, new package.
- **Sample:** `sample/Frank.Alps.Sample` demonstrates both exposures on a tic-tac-toe game resource — the app-wide document at `/.well-known/alps.json` and the per-resource excerpt negotiated at `/games/{id}` — each advertised by its own `Link` header (app-wide and resource-scoped respectively).
