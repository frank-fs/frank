...
let tests =
    testList
        "ValidateCommand"
        [ testCase "valid state with no issues"
          <| fun _ ->
              ...
              try
                  ...
                  let setupShapes (graph: IGraph) =
                      ...
                      assertTriple
                          graph
                          (shapeNode, targetClass, createUriNode graph (Uri "http://example.org/types/Product"))
                  ...
                  match result with
                  | Result.Error e -> failtest $"Expected Ok but got Error: {e}"
                  | Result.Ok r ->
          ...
          testCase "warns when class has no properties"
          <| fun _ ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              Directory.CreateDirectory(tempDir) |> ignore
          ...
          testCase "errors when shape targets non-existent class"
          <| fun _ ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  let setupShapes (graph: IGraph) =
                      ...
                      assertTriple
                          graph
                          (shapeNode, targetClass, createUriNode graph (Uri "http://example.org/types/Ghost"))
                  ...
                  | Result.Ok r ->
                      ...
                      let errors = r.Issues |> List.filter (fun i -> i.Severity = Severity.Error)
          ...
          testCase "warns for unmapped types and computes coverage"
          <| fun _ ->
              let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
              Directory.CreateDirectory(tempDir) |> ignore
              ...
              try
                  ...
                  | Result.Ok r ->
                      ...
                      let unmappedWarnings =
                          r.Issues |> List.filter (fun i -> i.Message.Contains("Widget"))
