...
[<Tests>]
let tests =
    testList "CompileCommand" [
        testCase "generates ontology, shapes, and manifest files" <| fun _ ->
            ...
            try
                ...
                | Ok r ->
                    ...
                    Expect.equal (root.GetProperty("baseUri").GetString()) "http://example.org/" "BaseUri should match"
                    Expect.equal (root.GetProperty("sourceHash").GetString()) "abc123" "SourceHash should match"
            finally
                if Directory.Exists tempDir then
                    Directory.Delete(tempDir, true)

        testCase "uses default output dir when none specified" <| fun _ ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
            Directory.CreateDirectory(tempDir) |> ignore
