...
let iriMatchTests =
    testList
        ...
        [ test "iri arm for Game maps to https://schema.org/Game" {
              let src =
                  unwrapOk (SemanticModelEmitter.emit "Probe.Generated" probeRegistry probeLock)

              Expect.stringContains src "| Game -> \"https://schema.org/Game\"" "Game iri arm"
          }

          test "iri arm for Holder maps to https://schema.org/Holder" {
              let src =
                  unwrapOk (SemanticModelEmitter.emit "Probe.Generated" probeRegistry probeLock)

              Expect.stringContains src "| Holder -> \"https://schema.org/Holder\"" "Holder iri arm"
          } ]
