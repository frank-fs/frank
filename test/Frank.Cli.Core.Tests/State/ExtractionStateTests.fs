...
[<Tests>]
let tests =
    testList "ExtractionState" [
        testCase "save and load roundtrip preserves triple count" <| fun _ ->
            let ontology = createGraph ()
            let s = createUriNode ontology (Uri "http://example.org/Person")
            let rdfType = createUriNode ontology (Uri "http://www.w3.org/1999/02/22-rdf-syntax-ns#type")
            let owlClass = createUriNode ontology (Uri "http://www.w3.org/2002/07/owl#Class")
            assertTriple ontology (s, rdfType, owlClass)

            let label = createUriNode ontology (Uri "http://www.w3.org/2000/01/rdf-schema#label")
            let labelVal = createLiteralNode ontology "Person" None
            assertTriple ontology (s, label, labelVal)

            let comment = createUriNode ontology (Uri "http://www.w3.org/2000/01/rdf-schema#comment")
            ...
            let state =
                { Ontology = ontology
                  ...
                  SourceMap =
                    ...
                    d.[Uri "http://example.org/Person"] <- { File = "Models.fs"; Line = 10; Column = 5 }
                    d
                  ...
                  UnmappedTypes =
                    [ { TypeName = "CustomWidget"
                        ...
                        Location = { File = "Widgets.fs"; Line = 3; Column = 1 } } ] }

            let tmpPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString(), "state.json")
        ...
        testCase "save and load roundtrip preserves metadata" <| fun _ ->
            let state =
            ...
            try
                ...
                let loaded = ExtractionState.load tmpPath |> Result.defaultWith (fun _ -> failtest "unreachable")
        ...
        testCase "load on non-existent path returns Error" <| fun _ ->
            let result = ExtractionState.load "/nonexistent/path/state.json"
            Expect.isError result "Should return Error for missing file"

        testCase "defaultStatePath returns expected path" <| fun _ ->
            let path = ExtractionState.defaultStatePath "/my/project"
            Expect.stringEnds path (Path.Combine("obj", "frank-cli", "state.json")) "Should end with obj/frank-cli/state.json"
    ]
