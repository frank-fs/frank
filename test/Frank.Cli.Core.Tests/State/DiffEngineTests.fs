...
let tests =
    testList
        "DiffEngine"
        [ testCase "detects added and removed triples"
          <| fun _ ->
          ...
          testCase "detects modification (same subject+predicate, different object)"
          <| fun _ ->
              let oldGraph = createGraph ()
          ...
          testCase "empty graphs produce no changes"
          <| fun _ ->
              let g1 = createGraph ()
              let g2 = createGraph ()
          ...
          testCase "formatDiff produces readable output"
          <| fun _ ->
              let diff =
                  { Added =
                      [ { Type = Added
