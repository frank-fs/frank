...
let tests =
    testList "FSharpRdf" [
        testCase "create graph and assert triples, query with triplesWithSubject" <| fun _ ->
            ...
            let obj2 = createLiteralNode graph "30" (Some(Uri "http://www.w3.org/2001/XMLSchema#integer"))
        ...
        testCase "getNode returns Some for existing URI" <| fun _ ->
            let graph = createGraph ()
        ...
        testCase "DU roundtrip: LiteralNode without datatype" <| fun _ ->
            let graph = createGraph ()
        ...
        testCase "DU roundtrip: LiteralNode with datatype" <| fun _ ->
            let graph = createGraph ()
        ...
        testCase "DU roundtrip: BlankNode" <| fun _ ->
            let graph = createGraph ()
        ...
        testCase "triplesWithPredicate returns matching triples" <| fun _ ->
            let graph = createGraph ()
            let pred = createUriNode graph (Uri "http://example.org/name")
