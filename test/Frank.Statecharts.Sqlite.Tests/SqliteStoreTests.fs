...
let makeStore () =
    let logger = TestLogger<SqliteStateMachineStore<TestState, int>>()

    let store =
        new SqliteStateMachineStore<TestState, int>("Data Source=:memory:", logger, jsonOptions)
...
let makeFileStore (path: string) =
    ...
    let store =
        new SqliteStateMachineStore<TestState, int>(connStr, logger, jsonOptions)
    ...
    testList
        ...
        [ testAsync "GetState returns None for unknown instance" {
          ...
          testAsync "SetState then GetState returns stored value" {
              ...
              Expect.equal result (Some(Active, 42)) "should return stored state"
          }
          ...
          testAsync "SetState overwrites previous value" {
              ...
              do! iface.SetState "game1" Active 1 |> Async.AwaitTask
              do! iface.SetState "game1" Completed 2 |> Async.AwaitTask
              let! result = iface.GetState("game1") |> Async.AwaitTask
              Expect.equal result (Some(Completed, 2)) "should return latest state"
          }
          ...
          testAsync "Multiple instances are independent" {
              ...
              do! iface.SetState "a" Active 1 |> Async.AwaitTask
              do! iface.SetState "b" Completed 2 |> Async.AwaitTask
              let! a = iface.GetState("a") |> Async.AwaitTask
              let! b = iface.GetState("b") |> Async.AwaitTask
              Expect.equal a (Some(Active, 1)) "instance a"
              Expect.equal b (Some(Completed, 2)) "instance b"
          }
          ...
          testAsync "Parameterized state (Error of string) round-trips correctly" {
              ...
              do! iface.SetState "err1" (Error "timeout") 99 |> Async.AwaitTask
              let! result = iface.GetState("err1") |> Async.AwaitTask
              Expect.equal result (Some(Error "timeout", 99)) "parameterized DU should round-trip"
          } ]
    ...
    testList
        ...
        [ testAsync "State survives store restart" {
              ...
              try
                  ...
                  let iface1 = store1 :> IStateMachineStore<TestState, int>
                  do! iface1.SetState "game1" Active 42 |> Async.AwaitTask
                  (store1 :> IDisposable).Dispose()

                  // Second store: read state (simulates restart)
                  let store2, _ = makeFileStore tempDb
                  let iface2 = store2 :> IStateMachineStore<TestState, int>
          ...
          testAsync "Parameterized state survives restart" {
              ...
              try
                  let store1, _ = makeFileStore tempDb
                  let iface1 = store1 :> IStateMachineStore<TestState, int>
                  do! iface1.SetState "err1" (Error "disk_full") 500 |> Async.AwaitTask
                  (store1 :> IDisposable).Dispose()

                  let store2, _ = makeFileStore tempDb
                  let iface2 = store2 :> IStateMachineStore<TestState, int>
                  let! result = iface2.GetState("err1") |> Async.AwaitTask

                  Expect.equal
                      result
                      (Some(Error "disk_full", 500))
                      "parameterized state should survive restart"
          ...
          testAsync "Multiple instances survive restart" {
              ...
              try
                  let store1, _ = makeFileStore tempDb
                  let iface1 = store1 :> IStateMachineStore<TestState, int>
                  do! iface1.SetState "a" Active 1 |> Async.AwaitTask
                  do! iface1.SetState "b" Completed 2 |> Async.AwaitTask
                  do! iface1.SetState "c" (Error "oops") 3 |> Async.AwaitTask
                  (store1 :> IDisposable).Dispose()

                  let store2, _ = makeFileStore tempDb
                  let iface2 = store2 :> IStateMachineStore<TestState, int>
                  let! a = iface2.GetState("a") |> Async.AwaitTask
                  let! b = iface2.GetState("b") |> Async.AwaitTask
                  let! c = iface2.GetState("c") |> Async.AwaitTask
                  Expect.equal a (Some(Active, 1)) "instance a should survive"
                  Expect.equal b (Some(Completed, 2)) "instance b should survive"
                  Expect.equal c (Some(Error "oops", 3)) "instance c should survive"
                  (store2 :> IDisposable).Dispose()
              finally
                  try
                      File.Delete(tempDb)
          ...
          testAsync "Overwritten state reflects latest value after restart" {
              ...
              try
                  let store1, _ = makeFileStore tempDb
                  let iface1 = store1 :> IStateMachineStore<TestState, int>
                  do! iface1.SetState "game1" Active 1 |> Async.AwaitTask
                  do! iface1.SetState "game1" Completed 99 |> Async.AwaitTask
                  (store1 :> IDisposable).Dispose()

                  let store2, _ = makeFileStore tempDb
                  let iface2 = store2 :> IStateMachineStore<TestState, int>
                  let! result = iface2.GetState("game1") |> Async.AwaitTask
                  Expect.equal result (Some(Completed, 99)) "should reflect latest overwrite"
                  (store2 :> IDisposable).Dispose()
              finally
                  try
                      File.Delete(tempDb)
...
let subscriptionTests =
    testList
        ...
        [ testAsync "Subscribe to existing state emits current state immediately" {
              ...
              do! iface.SetState "inst1" Active 10 |> Async.AwaitTask

              let received = ResizeArray<TestState * int>()

              let observer =
                  { new IObserver<TestState * int> with
          ...
          testAsync "Unsubscribe stops notifications" {
              ...
              let sub = iface.Subscribe "inst1" observer
              do! iface.SetState "inst1" Active 1 |> Async.AwaitTask

              sub.Dispose()
              // Small delay to let Unsubscribe message be processed
              do! Async.Sleep 50

              do! iface.SetState "inst1" Completed 2 |> Async.AwaitTask
...
let concurrencyTests =
    testList
        ...
        [ testAsync "Concurrent SetState operations complete without error" {
              let store, _ = makeStore ()
              use _s = store :> IDisposable
              ...
              let ops =
                  [| for i in 0..49 ->
                         async {
                             do!
                                 iface.SetState (sprintf "inst-%d" (i % 10)) Active i
                                 |> Async.AwaitTask
                         } |]
          ...
          testAsync "Concurrent GetState during SetState does not corrupt data" {
              ...
              do! iface.SetState "inst1" Active 0 |> Async.AwaitTask

              let ops =
                  [| for i in 0..99 ->
                         if i % 2 = 0 then
                             async { do! iface.SetState "inst1" Active i |> Async.AwaitTask }
                         else
                             async {
          ...
          testAsync "Concurrent writes to same instance produce consistent final state" {
              ...
              let ops =
                  [| for i in 0..99 ->
                         async { do! iface.SetState "race" Active i |> Async.AwaitTask } |]

              do! Async.Parallel ops |> Async.Ignore

              let! result = iface.GetState("race") |> Async.AwaitTask
              let _, ctx = Expect.wantSome result "should have a final state"

              // The final context value should be one of the values [0..99]
              // (whichever was last processed by the actor)
              Expect.isTrue (ctx >= 0 && ctx <= 99) "context should be a valid value from the writes"
          } ]
    ...
    testList
        ...
        [ testAsync "Dispose sends OnCompleted to subscribers" {
          ...
          test "Disposed store throws ObjectDisposedException on SetState" {
              ...
              Expect.throwsT<ObjectDisposedException>
                  (fun () -> iface.SetState "inst1" Active 1 |> ignore)
                  "should throw ObjectDisposedException"
          }
...
let diRegistrationTests =
    testList
        ...
        [ test "AddSqliteStateMachineStore registers IStateMachineStore singleton" {
              ...
              services.AddSqliteStateMachineStore<TestState, int>("Data Source=:memory:", jsonOptions)
              |> ignore

              use sp = (services :?> ServiceCollection).BuildServiceProvider()
              let store = sp.GetRequiredService<IStateMachineStore<TestState, int>>()
              Expect.isNotNull (box store) "should resolve IStateMachineStore from DI"
    ...
    testList
        ...
        [ testAsync "Schema is auto-created on first use with file database" {
              ...
              try
                  ...
                  do! iface.SetState "test1" Active 1 |> Async.AwaitTask
                  let! result = iface.GetState("test1") |> Async.AwaitTask
                  Expect.equal result (Some(Active, 1)) "should work with auto-created schema"
