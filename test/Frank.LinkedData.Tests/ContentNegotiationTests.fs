                ...
                webBuilder
                    ...
                    .Configure(fun (app: IApplicationBuilder) ->
                        ...
                        app.UseEndpoints(fun endpoints ->
                            let ep = endpoints.MapGet("/person/1", RequestDelegate(fun ctx ->
...
let private makeConfig (ontology: IGraph) =
    ...
    { OntologyGraph = ontology
      ...
      BaseUri = "http://example.org/api"
      Manifest = manifest } : LinkedDataConfig

let private makeOntologyWithNameAndAge () =
    let ontology = new Graph()
    let nameNode = ontology.CreateUriNode(UriFactory.Root.Create("http://example.org/api/properties/Person/Name"))
    let ageNode = ontology.CreateUriNode(UriFactory.Root.Create("http://example.org/api/properties/Person/Age"))
    let rdfType = ontology.CreateUriNode(UriFactory.Root.Create("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"))
    let owlProp = ontology.CreateUriNode(UriFactory.Root.Create("http://www.w3.org/2002/07/owl#DatatypeProperty"))
...
let tests =
    testList "ContentNegotiation" [
        testCase "LinkedDataMarker is added to endpoint metadata" <| fun _ ->
            let spec =
                ResourceSpec.Empty
        ...
        testCase "negotiateRdfType returns text/turtle for Accept: text/turtle" <| fun _ ->
            let result = negotiateRdfType "text/turtle"
            Expect.equal result (Some "text/turtle") "Should negotiate text/turtle"

        testCase "negotiateRdfType returns application/ld+json for Accept: application/ld+json" <| fun _ ->
        ...
        testCase "negotiateRdfType returns application/rdf+xml for Accept: application/rdf+xml" <| fun _ ->
        ...
        testCase "negotiateRdfType returns None for Accept: application/json" <| fun _ ->
            let result = negotiateRdfType "application/json"
            Expect.equal result None "Should not negotiate application/json as RDF"

        testCase "negotiateRdfType returns None for empty Accept" <| fun _ ->
            let result = negotiateRdfType ""
            Expect.equal result None "Should not negotiate empty Accept"

        testCase "projectJsonToRdf creates triples from JSON matching ontology" <| fun _ ->
        ...
        testCase "projectJsonToRdf ignores properties not in ontology" <| fun _ ->
            let ontology = new Graph()
            let nameNode = ontology.CreateUriNode(UriFactory.Root.Create("http://example.org/api/properties/Person/Name"))
            let rdfType = ontology.CreateUriNode(UriFactory.Root.Create("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"))
            let owlProp = ontology.CreateUriNode(UriFactory.Root.Create("http://www.w3.org/2002/07/owl#DatatypeProperty"))
            ...
            let json = System.Text.Json.JsonDocument.Parse("""{"Name":"Bob","Unknown":"value"}""")
        ...
        testCase "writeRdf dispatches to correct formatter" <| fun _ ->
            let g = new Graph()
        ...
        testCase "middleware passes through when no LinkedDataMarker" <| fun _ ->
            let ontology = makeOntologyWithNameAndAge ()
        ...
        testCase "middleware intercepts when LinkedDataMarker is present and Accept is text/turtle" <| fun _ ->
            let ontology = makeOntologyWithNameAndAge ()
            let config = makeConfig ontology
            ...
            Expect.isFalse (body.StartsWith("{")) (sprintf "Body should not be JSON. Got: %s" (body.Substring(0, min body.Length 500)))
            Expect.equal (response.Content.Headers.ContentType.MediaType) "text/turtle"
        ...
        testCase "middleware passes through when Accept is not RDF" <| fun _ ->
            let ontology = makeOntologyWithNameAndAge ()
            let config = makeConfig ontology
            use host = createTestHost config true
            let server = host.GetTestServer()
