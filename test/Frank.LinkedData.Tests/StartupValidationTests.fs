...
let tests =
    testList
        "StartupValidation"
        [ testCase "valid configuration starts successfully via TestHost"
          <| fun _ ->
              ...
              let s =
                  ontology.CreateUriNode(UriFactory.Root.Create("http://example.org/api/properties/Test/Name"))
          ...
          testCase "missing embedded resources produces descriptive error"
          <| fun _ ->
          ...
          <| fun _ ->
              ...
              | Error msg ->
                  ...
                  Expect.stringContains
                      ex.Message
                      "LinkedData configuration error"
          ...
          <| fun _ ->
              // The useLinkedData CE operation calls Assembly.GetEntryAssembly() which can return null
              // in test host scenarios. The code should failwith a descriptive message suggesting
              // useLinkedDataWith as an alternative.
              // We can't easily force GetEntryAssembly to return null, but we can verify the
              // error message format by testing the wrapping directly.
              let errorMsg =
              ...
              // Verify the message mentions the alternative
              Expect.stringContains errorMsg "useLinkedDataWith" "Error should mention useLinkedDataWith alternative"
              Expect.stringContains errorMsg "test host" "Error should mention test host scenarios"
              Expect.stringContains errorMsg "GetEntryAssembly()" "Error should mention the failing API"

              // Verify useLinkedDataWith works as the recommended alternative
              let ontology = new Graph()
              ...
              // useLinkedDataWith bypasses GetEntryAssembly entirely
              let hostBuilder =
                  HostBuilder()
                      .ConfigureWebHost(fun webBuilder ->
                          webBuilder
              ...
              use client = server.CreateClient()
              let response: HttpResponseMessage = client.GetAsync("/test").Result
              Expect.equal (int response.StatusCode) 200 "useLinkedDataWith should work without GetEntryAssembly"

          testCase "app without useLinkedData starts normally with no false positives"
          <| fun _ ->
                          ...
                          webBuilder
                              ...
                              .Configure(fun (app: IApplicationBuilder) ->
                                  ...
                                  app.UseEndpoints(fun endpoints ->
                                      endpoints.MapGet(
                                          "/hello",
                                          RequestDelegate(fun ctx -> ctx.Response.WriteAsync("world"))
                                      )
                                      |> ignore)
                                  |> ignore)
                          |> ignore)
