...
let private createRealisticState (projectDir: string) =
    ...
    assertTriple ontology (priceNode, rangeNode, createUriNode ontology (Uri "http://www.w3.org/2001/XMLSchema#decimal"))
    ...
    let productShape = createUriNode shapes (Uri "http://example.org/shapes/ProductShape")
...
let tests =
    testList "Pipeline Integration" [
        testCase "validate → compile pipeline produces valid artifacts" <| fun _ ->
        ...
        testCase "validate → compile → diff pipeline detects changes" <| fun _ ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
            Directory.CreateDirectory(tempDir) |> ignore
            ...
            try
                ...
                let newClass = createUriNode state2.Ontology (Uri "http://example.org/types/Customer")
        ...
        testCase "validate reports issues for inconsistent state" <| fun _ ->
            let tempDir = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString())
            Directory.CreateDirectory(tempDir) |> ignore
            ...
            try
                ...
                let badShape = createUriNode state.Shapes (Uri "http://example.org/shapes/GhostShape")
                assertTriple state.Shapes (badShape, shRdfType, nodeShape)
                assertTriple state.Shapes (badShape, targetClass, createUriNode state.Shapes (Uri "http://example.org/types/NonExistent"))
                ...
                | Ok vr ->
                    ...
                    let errors = vr.Issues |> List.filter (fun i -> i.Severity = "error")
